...
for nprocs in "${PROCESS_COUNTS[@]}"; do
    ...
    if [ "$EXECUTOR_TYPE" == "thread" ]; then
        ...
        sbatch -N $nodes -n $nprocs --job-name="eval_mgr_mpi${nprocs}" test_multinode_mpi.sh
    fi
...
echo "  tail -f logs/eval_mgr_*.out"
echo ""
echo "View results when complete:"
echo "  grep -H 'Total time\|Workers:' logs/eval_mgr_*.out | sort"
