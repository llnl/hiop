...
\Hibbpy offers two means to utilize parallelism in the context of Bayesian optimization. The first means is the parallel evaluation of the black-box objective function on a batch of points that arises in batched Bayesian optimization methods as well as in the generation of initial training points to train the Gaussian process surrogate on. The second means is the parallel evaluation of an optimization algorithm, which converges to locally optimal points, on a batch of random starting points in order to estimate a global optima. Both are included below, the former being \verb~obj_evaluator~ and the latter being \verb~opt_evaluator~.
...
\begin{verbatim}
> env MPI4PY_FUTURES_MAX_WORKERS=4 mpirun -np 1 python BODriver.py 
\end{verbatim}
